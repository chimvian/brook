...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget -N -O brook "https://github.com/chimvian/brook/raw/master/abccea/brook"
	else
		wget -N -O brook_linux_386 "https://github.com/chimvian/brook/raw/master/abccea/brook_linux_386"
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then                                   
		if ! wget -N -O brook-pf_centos https://github.com/chimvian/brook/raw/master/abccea/brook-pf_centos -O /etc/init.d/brook-pf; then
		...
		if ! wget -N -O brook-pf_debian https://github.com/chimvian/brook/raw/master/abccea/brook-pf_debian -O /etc/init.d/brook-pf; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/brook-pf

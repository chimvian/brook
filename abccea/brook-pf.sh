...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/chimvian/brook/master/abccea/brook-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/brook-pf" ]]; then
		rm -rf /etc/init.d/brook-pf
		Service_brook
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/chimvian/brook/master/abccea/brook-pf.sh" && chmod +x brook.sh
